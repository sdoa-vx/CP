// ──────────────────────────────────────────────────────────────────
// File:    SysProvisionModel.workflow.js
// Version: 2.0.0
...
class SysProvisionModelWorkflow extends WorkflowBase {
    ...
    async run(context = {}) {
        ...
        Middleware.log(`[SysProvisionModel] Starting setup — python: ${pyInfo.exe}`);
        Middleware.log(`[SysProvisionModel] Venv target: ${venvPath}`);
        Middleware.log(`[SysProvisionModel] Model: ${modelName}`);
        ...
        return new Promise((resolve) => {
            ...
            proc.stdout.on("data", (chunk) => {
                ...
                for (const line of lines) {
                    ...
                    if (parsed.done) {
                        ...
                        Middleware.log(`[SysProvisionModel] [${parsed.step}/${parsed.total}] ${parsed.label}${sub}`);
                    }
                }
            });
            ...
            proc.on("exit", (code) => {
                if (code === 0 && !lastProgress?.done) {
                    // bootstrap exited cleanly but we may have already resolved via "done" event
                    // Just in case:
                    Middleware.log("[SysProvisionModel] Process exited with code 0");
                } else if (code !== 0) {
                    const err = `Bootstrap exited with code ${code}`;
                    this._emit("sys:provision:error", { error: err });
                    this._writeStatus(statusFile, { state: "error", error: err, completedAt: new Date().toISOString() });
