...
function resolveDependencyGraph(fileTiers, projectFiles, projectDir) {
  ...
  // Circuit breaker: if there are too many eager files, skip expensive import traversal
  if (queue.length > 50) return escalationMap;
...
class FileContextWorkflow {
    ...
    const MAX_BYTES = maxBytes || 24576; // 24KB — ~6k tokens, safe for any context window
    const MAX_FILE_BYTES = 8192;   // 8KB per-file cap
    let totalBytes = 0;
    ...
    function walkSync(dir, filelist = []) {
      ...
      files.forEach(file => {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
          const skipDirs = ["node_modules", "dist", "target", "build", "coverage", "venv", ".venv", "__pycache__"];
    ...
    try {
      ...
      if (vfsIndex && Array.isArray(vfsIndex.entries) && vfsIndex.entries.length > 0) {
        ...
        for (const entry of vfsIndex.entries) {
          count++;
          if (count > 800) {
            lines.push(`... and ${vfsIndex.entries.length - 800} more files indexed ...`);
            break;
          }
