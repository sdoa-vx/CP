// ──────────────────────────────────────────────────────────────────
// File:    AutoOptimizeModels.workflow.js
// Version: 1.1.0
...
class AutoOptimizeModelsWorkflow extends WorkflowBase {
    ...
    async run(payload) {
        try {
            ...
            // Exclude OpenRouter's own meta-router models (openrouter/free, openrouter/auto,
            // openrouter/owl-alpha, etc.) — they appear in the API but return 400 when used
            // directly for chat completions. Also exclude non-text modalities (music, audio).
            const EXCLUDED_PREFIXES = ["openrouter/"];
            const EXCLUDED_MODALITIES = ["audio", "music"];
            const freeModels = allModels.filter(m => {
                const p = m.pricing;
                if (!p || p.prompt !== "0" || p.completion !== "0") return false;
                if (EXCLUDED_PREFIXES.some(prefix => m.id.startsWith(prefix))) return false;
                const modality = (m.architecture?.modality || "").toLowerCase();
                if (EXCLUDED_MODALITIES.some(ex => modality === ex)) return false;
                return true;
            });
