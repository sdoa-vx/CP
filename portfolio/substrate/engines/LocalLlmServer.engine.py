# ──────────────────────────────────────────────────────────────────
# File:    LocalLlmServer.engine.py
# Version: 5.0.0
# Updated: 2026-06-17T00:00:00Z
# Changes: Relocated to canonical sdoavx/ structure
# ──────────────────────────────────────────────────────────────────
# Last modified: 2026-06-03 05:30 UTC
# ============================================================
# LocalLlmServer.engine.py — SDOA v5.0 Engine (Python)
# version: 5.0.0
# ============================================================

# MANIFEST = {
#   id: "LocalLlmServer.engine",
#   type: "engine",
#   layer: 3,
#   runtime: "Python",
#   version: "5.0.0",
...
#   requires: [],
#   capabilities: ["local_inference_server", "http_endpoint_generation"],
#   dependencies: [],
#   optimization: {
...
#   },
#   docs: {
#     description: "Local Qwen Server AI engine running on Python ThreadingHTTPServer.",
#     author: "ProtoAI team",
#     sdoa: "5.0.0"
#   }
# };
...
def load_model():
    ...
    try:
        ...
        print(f"[LocalLlmServer.engine] Loading {MODEL_NAME} on {_device} ({dtype})...", flush=True)
        t0 = time.time()
        ...
        print(f"[LocalLlmServer.engine] Model ready in {elapsed:.1f}s on {_device}", flush=True)

    except Exception as e:
        _load_error = str(e)
        print(f"[LocalLlmServer.engine] ERROR loading model: {e}", flush=True)
    finally:
        _loading = False
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            print(f"[LocalLlmServer.engine] generate error: {e}", flush=True)
...
def main():
    parser = argparse.ArgumentParser(description="ProtoAI local inference engine")
    ...
    try:
        ...
        print("[LocalLlmServer.engine] Shutting down.", flush=True)
