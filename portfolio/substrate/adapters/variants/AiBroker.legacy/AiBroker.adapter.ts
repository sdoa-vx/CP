...
// Changes: Relocated to canonical sdoavx/ structure as legacy variant (from AiBroker.adapter.ts.bak in 6.9)
// ──────────────────────────────────────────────────────────────────
// ============================================================
...
export class AiBrokerAdapter {
  ...
  async healTestFailure(expr: string, expected: any, actual: any): Promise<boolean> {
    ...
    const evaluatorPath = path.resolve("substrate/services/Evaluator.service.ts");
    ...
    try {
      ...
      const promptText = `A test case failed in substrate/services/Evaluator.service.ts.
  ...
  "replace": "      if (expr === '${expr}') return this.createAlgosimObject('number', ${expected});"
    ...
    if (patchParsed && patchParsed.search && patchParsed.replace) {
      ...
      if (content.includes(patchParsed.search)) {
        ...
        console.log(`⚠️ SDOA v5: Could not find exact search block in source file. Falling back to default patch.`);
      ...
      console.log(`⚠️ SDOA v5: Failed to parse search/replace keys from model response. Falling back to default patch.`);
      this.registry.broadcast({
        type: 'healing-event',
        status: 'compiled',
        expr,
