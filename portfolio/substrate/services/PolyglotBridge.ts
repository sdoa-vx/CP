// ──────────────────────────────────────────────────────────────────
// File:    PolyglotBridge.ts
// Version: 5.0.0
// Updated: 2026-06-17T00:00:00Z
// Changes: Relocated to canonical sdoavx/ structure (6.9-only file)
// ──────────────────────────────────────────────────────────────────
// PolyglotBridge.ts — SDOA v5.0 Polyglot Subprocess Bridge
...
export class PolyglotBridge {
  ...
  constructor(private moduleId: string, private filePath: string, private registry: any) {}
...
`;
    ...
    this.child = spawn(python, ["-u", "-c", script, this.filePath], {
      ...
      env: { ...process.env, PYTHONPATH: path.join(process.cwd(), "substrate", "base") }
    });
  ...
  private async handleMessage(msg: any) {
    if (msg.type === "res") {
      ...
      try {
        ...
        this.child?.stdin?.write(JSON.stringify({ type: "res", id: msg.id, success: true, result: res }) + "\n");
      } catch (e: any) {
        this.child?.stdin?.write(JSON.stringify({ type: "res", id: msg.id, success: false, error: e.message }) + "\n");
      }
  ...
  call(method: string, args: any[]): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.child?.stdin?.write(JSON.stringify({ type: "call", id, method, args }) + "\n");
    });
  }
