// ──────────────────────────────────────────────────────────────────
// File:    PersistentMemory.service.js
// Version: 5.0.0
...
"use strict";
// ============================================================
// PersistentMemory.service.js — SDOA v5.0 Service
// version: 5.0.0
...
class PersistentMemoryService {
    async loadMemoryPrompt(project) {
        try {
            ...
            if (traits)
                memLines.push(`User Traits: ${traits}`);
            if (prefs)
                memLines.push(`User Preferences: ${prefs}`);
            if (projSummary)
                memLines.push(`Project Summary: ${projSummary}`);
            if (projTags)
                memLines.push(`Project Tags: ${projTags}`);
            if (projConstraints)
                memLines.push(`Project Constraints: ${projConstraints}`);
...
PersistentMemoryService.MANIFEST = {
    ...
    requires: [],
    dependencies: [],
    capabilities: ["memory:load-prompt-context"],
    optimization: {
