...
(function () {
    ...
    function _buildBodyLayout() {
        ...
        if (_activeTab === "browse") {
            ...
            `;
            if (typeof window.FileTree?.render === "function") window.FileTree.render(body.querySelector("#filemgrTreeInner"), _rootPath);
            if (typeof window.FileList?.render === "function") window.FileList.render(body.querySelector("#filemgrListPane"));
            if (typeof window.FileList?.loadPath === "function") window.FileList.loadPath(_rootPath);
            if (typeof window.ManifestPanel?.render === "function") window.ManifestPanel.render(body.querySelector("#filemgrManifestPane"));
        } else {
            _renderVfsTab(body);
        }
    }
    ...
    function _wireToolbar() {
        ...
        _container?.querySelector("#filemgrGoUp")?.addEventListener("click", () => {
            ...
            if (parent && parent !== _rootPath) {
                _rootPath = parent;
                if (typeof window.FileTree?.setRootPath === "function") window.FileTree.setRootPath(_rootPath);
                if (typeof window.FileList?.loadPath === "function") window.FileList.loadPath(_rootPath);
                _updateBreadcrumb(_rootPath);
            }
        });
        _container?.querySelector("#filemgrRefresh")?.addEventListener("click", () => {
            if (typeof window.FileTree?.refresh === "function") window.FileTree.refresh();
            if (typeof window.FileList?.refresh === "function") window.FileList.refresh();
        });
        _container?.querySelector("#filemgrGoHome")?.addEventListener("click",  async () => {
            _rootPath = await _getProjectDir();
            if (typeof window.FileTree?.setRootPath === "function") window.FileTree.setRootPath(_rootPath);
            if (typeof window.FileList?.loadPath === "function") window.FileList.loadPath(_rootPath);
            _updateBreadcrumb(_rootPath);
    ...
    function _wireBusEvents() {
        ...
        bus.on("filetree:folderSelected",   ({ path })        => {
            if (typeof window.FileList?.loadPath === "function") window.FileList.loadPath(path);
    ...
    function setRootPath(path) {
        _rootPath=path;
        if (typeof window.FileTree?.setRootPath === "function") window.FileTree.setRootPath(path);
        if (typeof window.FileList?.loadPath === "function") window.FileList.loadPath(path);
        _updateBreadcrumb(path);
    }
    function refresh() {
