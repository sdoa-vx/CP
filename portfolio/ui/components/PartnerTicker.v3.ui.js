...
(function () {
    ...
    const MANIFEST = {
        ...
        docs: { description: "Undeclared duplicate of the canonical PartnerTicker.v3.ui module (also present in adapters/ and features/PartnerTicker/); pending consolidation in Phase 4. Local model activity ticker. Minimized strip → hover preview → locked panel. Replays orchestrator events from each completed request." },
            author: "ProtoAI team",
        actions: {
            commands: {
    ...
    const EVENT_META = {
        ...
        "orchestrator:routed":     { icon: "[ok]", label: (d) => `-> ${d.profile || "default"} (${d.complexity || "?"})`, color: "ok"  },
        "orchestrator:engineering":{ icon: "[e]",  label: (d) => "engineering prompt...",                  color: "dim"    },
        "orchestrator:engineered": { icon: "[ok]", label: (d) => `optimised ${d.originalLen}->${d.optimizedLen} chars`,   color: "ok"  },
        "orchestrator:watching":   { icon: "[w]",  label: (d) => `watching (${d.bufferLen} chars)...`,     color: "dim"    },
        "orchestrator:flagged":    { icon: "[!]",  label: (d) => `flagged: ${(d.flag || "").slice(0,60)}`, color: "warn"   },
        "orchestrator:auditing":   { icon: "[a]",  label: (d) => "auditing response...",                   color: "dim"    },
        "orchestrator:audited":    { icon: "[ok]", label: (d) => `score ${d.score ?? "?"}/10 — ${(d.note || "").slice(0,40)}`, color: "ok" },
        "orchestrator:commentary_generating": { icon: "[t]", label: (d) => `thinking (${d.persona})...`, color: "dim" },
        "orchestrator:commentary": { icon: "[c]",  label: (d) => d.text, color: "persona" },
        "orchestrator:error":      { icon: "[x]",  label: (d) => `[${d.stage}] ${(d.message||"").slice(0,60)}`, color: "err" },
        "local:modelLoaded":       { icon: "[m]",  label: (d) => "local model ready",                     color: "ok"     },
        "route_error":             { icon: "[x]",  label: (d) => `route error: ${(d.error||"").slice(0,50)}`, color: "err" },
        "audit_error":             { icon: "[x]",  label: (d) => `audit error: ${(d.error||"").slice(0,50)}`, color: "err" },
    };
