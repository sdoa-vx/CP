...
(function () {
    ...
    function _renderNotInVfs(filePath) {
        ...
        _container.innerHTML = `
            <div class="manifest-content">
                <div class="manifest-file-header">
                    <div class="manifest-file-icon">[f]</div>
                    <div class="manifest-file-info">
    ...
    function _renderPurposeFields(type, p) {
        ...
        if (type === "image") {
            if (p.width && p.height) add("Dimensions", `${p.width} x ${p.height}`);
            add("Format", p.format);
        }
        if (type === "audio") {
            add("Title",  p.title);
    ...
    function _typeIcon(type) {
        const m = { code: "[code]", document: "[doc]", data: "[data]", image: "[img]", audio: "[aud]", video: "[vid]", directory: "[dir]", generic: "[f]" };
        return m[type] || "[f]";
    }
