...
/**
 * sdoa-linter.js
 * Programmatic SDOA compliance checker for SDOAvX (v4 and v5).
 * Usage: node sdoa-linter.js <file-path>
 */

const fs = require("fs");
const path = require("path");

function logResult(file, ok, errors = [], warnings = []) {
  console.log(`=== SDOA LINTER: ${path.basename(file)} ===`);
...
function lintFile(filePath) {
  ...
  // Determine Layer/Type from file path or manifest contents
  ...
  if (detectedLayer === null) {
    ...
    return logResult(filePath, false, errors, warnings);
  }

  // 1. Line Limit Checks
  const limits = { 1: 200, 2: 150, 3: 200 };
  ...
  if (!isPython) {
    // Strip comments first in the first 5000 characters to prevent false missing strict errors
    const cleanPrefix = content.slice(0, 5000).replace(/\/\*[\s\S]*?\*\/|\/\/.*$/gm, "").trim();
    const hasStrict = cleanPrefix.match(/["']use strict["']/);

    // TypeScript files have implicit strict mode inside ES modules; pure JS must define it
  ...
  try {
    ...
    if (versionMatch && versionMatch[1] !== expectedVer) {
      // Relaxed check for non-primitives
  ...
  return logResult(filePath, errors.length === 0, errors, warnings);
}
...
if (require.main === module) {
  ...
  if (!file) {
    console.log("Usage: node sdoa-linter.js <file-path>");
    process.exit(1);
  }
  const ok = lintFile(path.resolve(file));
  process.exit(ok ? 0 : 1);
}
