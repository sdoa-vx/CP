// Registry.service.ts — SDOA v5.0 Service Registry
import { PolyglotBridge } from "../../substrate/services/PolyglotBridge";
...
export class Registry {
  static MANIFEST: SdoaManifest = {
    id: "Registry.service", type: "service", layer: 3, runtime: "NodeJS", version: "5.0.0",
  ...
  async registerWasm(manifest: SdoaManifest, wasmBuffer: Buffer, importObject: WebAssembly.Imports = {}): Promise<void> {
    const compiled = await WebAssembly.instantiate(wasmBuffer, importObject);
    this.classes.set(manifest.id, { MANIFEST: manifest });
  ...
  get<T = any>(id: string): T {
    ...
    if (!instance) throw new Error(`SDOA Error: Module '${id}' is not registered.`);
          ...
          return function (...args: any[]) {
            ...
            try {
              ...
              if (res instanceof Promise) {
                return res.then(
                  (val) => { self.broadcast({ type: "call-end", moduleId: id, commandName: prop, success: true, duration: Date.now() - t0 }); return val; },
                  (err) => { self.broadcast({ type: "call-end", moduleId: id, commandName: prop, success: false, error: String(err), duration: Date.now() - t0 }); throw err; }
                );
              }
  ...
  getManifest(id: string): SdoaManifest {
    ...
    if (!cls) throw new Error(`SDOA Error: Manifest for module '${id}' not found.`);
    return cls.MANIFEST;
  }
